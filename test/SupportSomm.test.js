...
const MasterSomm = artifacts.require('MasterSomm');
const VintageBar = artifacts.require('VintageBar');
const SupportSomm = artifacts.require('SupportSomm');
const MockBEP20 = artifacts.require('libs/MockBEP20');

contract('SupportSomm', ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
    this.syrup = await MockBEP20.new('LPToken', 'LP1', '1000000', {
      from: minter,
    });
    this.chef = await SupportSomm.new(this.syrup.address, '40', '300', '400', {
      from: minter,
    });
  });
  ...
  it('try syrup', async () => {
    ...
    this.syrup = await VintageBar.new(this.cake.address, { from: minter });
    this.lp1 = await MockBEP20.new('LPToken', 'LP1', '1000000', {
      from: minter,
    });
    this.chef = await MasterSomm.new(
      this.cake.address,
      this.syrup.address,
      dev,
      '1000',
    ...
    this.chef2 = await SupportSomm.new(this.syrup.address, '40', '600', '800', {
      from: minter,
    });
